...
public static class DependencyInjection
{
    private static IConfiguration Configuration =>
        field ??= new ConfigurationBuilder().AddJsonFile("appsettings.json", true).Build();

    private static IServiceProvider Provider =>
        field ??= Configure(new ServiceCollection(), Configuration).BuildServiceProvider();

    private static IServiceCollection Configure(IServiceCollection services, IConfiguration configuration) =>
        services
            .AddInfrastructures()
            .AddSingleton<ProcessContext>();
