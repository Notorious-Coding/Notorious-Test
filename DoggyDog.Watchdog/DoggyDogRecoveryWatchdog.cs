...
internal class DoggyDogRecoveryWatchdog
{
    ...
    public static void Banner(int pid, Guid environmentId)
    {
        ...
        {
            Console.WriteLine(
                $"                      Monitoring PID {pid}  ·  EID {environmentId}  ·  v{version.Major}.{version.Minor}.{version.Build}");
            Console.WriteLine();
        });
    }
    ...
    public async Task Run(int pid, Guid environmentId)
    {
        ...
        IReadOnlyList<InfrastuctureRegistryEntry> entries =
            (await _registry.GetByEnvironmentId(new EnvironmentId(environmentId))).ToList();
        ...
        {
            using LogScope scope = _logger.CreateScope(entry.InfrastructureType.Name);
    ...
    private IInfrastructureCleaner? RetrieveCleaner(InfrastuctureRegistryEntry entry)
    {
        ...
        CleanerAttribute? attr = infrastructureType?.GetCustomAttribute<CleanerAttribute>(true);
