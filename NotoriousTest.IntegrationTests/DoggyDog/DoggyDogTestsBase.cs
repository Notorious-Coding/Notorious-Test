    ...
    : IntegrationTest<NotoriousTestEnvironment>(environment)
{
    ...
    public async Task DoggyDog_Should_Do_Nothing_When_Process_Exit_Normally()
    {
        ...
        Process? process =
            DoggyDogTestFramework.Arrange.StartFakeProcess(environmentId, timeToExit: FAKE_PROCESS_WAIT_TIME);

        SqliteInfrastructure registry = Environment.GetInfrastructure<SqliteInfrastructure>();

        (Process doggyDogProcess, StringBuilder stdoutBuilder) = DoggyDogTestFramework.Act.StartDoggyDog(process.Id,
            typeof(DoggyDogTestsBase).Assembly.Location, registry.GetDatabaseConnectionString(), environmentId);
    ...
    public async Task DoggyDog_Should_Cleanup_Orphan_Infrastructures_When_Process_Exit_Abnormally()
    {
        ...
        Process process = DoggyDogTestFramework.Arrange.StartFakeProcess(environmentId, 1, FAKE_PROCESS_WAIT_TIME);

        InfrastuctureRegistryEntry fakeEntry = await DoggyDogTestFramework.Arrange.CreateInfrastructureInRegistry(
            registry, process, typeof(DoggyDogTestFramework.Arrange.FakeInfrastructureWithCleaner), environmentId);
        ...
        await DoggyDogTestFramework.Assert.ShouldHaveFoundInfrastructureToClean(stdout, 1, environmentId);
    ...
    public async Task DoggyDog_Should_Exit_When_Monitored_Process_Not_Found()
    {
        ...
        (Process doggyDogProcess, StringBuilder stdoutBuilder) = DoggyDogTestFramework.Act.StartDoggyDog(PROCESS_ID,
            typeof(DoggyDogTestsBase).Assembly.Location, registry.GetDatabaseConnectionString(), environmentId);
    ...
    public async Task DoggyDog_Should_Exit_When_Registry_File_Not_Found()
    {
        ...
        (Process doggyDogProcess, StringBuilder stdoutBuilder) = DoggyDogTestFramework.Act.StartDoggyDog(process.Id,
            typeof(DoggyDogTestsBase).Assembly.Location, FAKE_CONNECTION_STRING, environmentId);
    ...
    public async Task DoggyDog_Should_Skip_Infrastructure_Without_Cleaner_Attribute_When_Process_Crashes()
    {
        ...
        InfrastuctureRegistryEntry fakeEntry = await DoggyDogTestFramework.Arrange.CreateInfrastructureInRegistry(
            registry, process, typeof(DoggyDogTestFramework.Arrange.FakeInfrastructureWithoutCleanerAttribute),
            environmentId);
    ...
    public async Task DoggyDog_Should_Cleanup_Multiple_Orphan_Infrastructures_When_Process_Crashes()
    {
        ...
        Process fakeProcess = DoggyDogTestFramework.Arrange.StartFakeProcess(environmentId, 1, FAKE_PROCESS_WAIT_TIME);

        InfrastuctureRegistryEntry fakeEntry1 = await DoggyDogTestFramework.Arrange.CreateInfrastructureInRegistry(
            registry, fakeProcess, typeof(DoggyDogTestFramework.Arrange.FakeInfrastructureWithCleaner), environmentId);
        InfrastuctureRegistryEntry fakeEntry2 = await DoggyDogTestFramework.Arrange.CreateInfrastructureInRegistry(
            registry, fakeProcess, typeof(DoggyDogTestFramework.Arrange.FakeInfrastructure2WithCleaner), environmentId);

        (Process doggyDogProcess, StringBuilder stdoutBuilder) = DoggyDogTestFramework.Act.StartDoggyDog(fakeProcess.Id,
            typeof(DoggyDogTestsBase).Assembly.Location, registry.GetDatabaseConnectionString(), environmentId);
        ...
        await DoggyDogTestFramework.Assert.ShouldHaveFoundInfrastructureToClean(stdout, 2, environmentId);
    ...
    public async Task DoggyDog_Should_Display_InfrastructureLifecycleEvents()
    {
        ...
        InfrastuctureRegistryEntry fakeEntry1 = await DoggyDogTestFramework.Arrange.CreateInfrastructureInRegistry(
            registry, fakeProcess, typeof(DoggyDogTestFramework.Arrange.FakeInfrastructureWithCleaner), environmentId);
        await DoggyDogTestFramework.Assert.ShouldHaveCatchInitEvent(stdoutBuilder,
            typeof(DoggyDogTestFramework.Arrange.FakeInfrastructureWithCleaner), TestContext.Current.CancellationToken);

        await DoggyDogTestFramework.Arrange.TriggerInfrastructureReset(registry, fakeEntry1.InfrastructureId);
        await DoggyDogTestFramework.Assert.ShouldHaveCatchResetEvent(stdoutBuilder,
            typeof(DoggyDogTestFramework.Arrange.FakeInfrastructureWithCleaner), TestContext.Current.CancellationToken);

        await DoggyDogTestFramework.Arrange.DestroyInfrastructureFromRegistry(registry, fakeEntry1.InfrastructureId);
        await DoggyDogTestFramework.Assert.ShouldHaveCatchDestroyEvent(stdoutBuilder,
            typeof(DoggyDogTestFramework.Arrange.FakeInfrastructureWithCleaner), TestContext.Current.CancellationToken);
