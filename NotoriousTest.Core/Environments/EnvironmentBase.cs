...
public abstract class EnvironmentBase
{
        ...
        ITestLogger logger, IServiceProvider serviceProvider)
    {
        ...
        IntegrationTestContext.Current.CurrentAssembly = CurrentAssembly;
    }
    ...
    /// </summary>
    /// <remarks>
    ///     This method sets up the service collection, configures infrastructure services,
    ///     builds the service provider, and initializes all registered infrastructure components in order. It should be
    ///     called before using any dependent services or infrastructure components.
    /// </remarks>
    ...
    private async Task StartDoggyDog()
    {
        RuntimeConfiguration? runtimeConfiguration =
            Runtime.GetSupportedRuntimes(IntegrationTestContext.Current.CurrentAssembly);
        if (runtimeConfiguration == null)
            Logger.Log(
                $"Runtime for {IntegrationTestContext.Current.CurrentAssembly.FullName} cannot be found. Cleaner resolution may not work properly.",
                EnvironmentId);

        IEnumerable<string>? runtimesPath = runtimeConfiguration?.SupportedFrameworks?.Select(sf => sf.FilePath);
        WatchDog.Start(IntegrationTestContext.Current.CurrentAssembly, Process.GetCurrentProcess().Id, EnvironmentId,
            runtimesPath);
    }
